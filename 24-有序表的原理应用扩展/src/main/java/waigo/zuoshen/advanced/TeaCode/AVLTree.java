package waigo.zuoshen.advanced.TeaCode;
		...
		while (node != null) {
			...
			if (nodeBalance == 2) {//R型
				if (node.right!=null&&
						...
						((AVLNode)(node.right.right)).height==leftHeight+1) {
					node = (AVLNode) avlRotateLeft(node);//RR,如果错误的那边左右子树高度相同，那么需要使用RR或者LL来改
					break;
				} else {
					node = (AVLNode) doubleRotateRightLeft(node);//RL
					break;
				}
			} else if (nodeBalance == -2) {//L型
				if (node.left!=null&&node.left.left!=null&&((AVLNode)node.left.left).height==rightHeight+1) {
					node = (AVLNode) avlRotateRight(node);//LL型
					break;
				} else {
					node = (AVLNode) doubleRotateLeftRight(node);//LR型
					break;
				}
			} else {
				updateHeight(node);
